...
//sent to the server - new player joined,only one time for a player
...
var numOfPlayersToStartAGame=2;
...
var myObstacles = [];
//will contain randoms numbers from server
...
var canPlay = false;
//True only in the first time the player connect
...
function ready() {
  ...
  if (document.getElementById("txtbox").value.trim() != "") {
    ...
    movement.rivalName = document.getElementById("rivalName").value;
    movement.gameCode = document.getElementById("gameCode").value;
...
socket.on('randoms', function (randomNumsArray) {
  ...
  //Change the bird image 3 times in 1 sec
...
//60 times in one sec, Get array of players with all details
socket.on('state', function (players) {
  myid = socket.id;

  var MatchBetweenPlayersFromFaceAfeka=true;
  ...
  for (var id in players) {
    ...
    //Check match between players from faceafeka
    if (myid != player.id)
    {
       if(movement.playername!=player.rivalName)
            MatchBetweenPlayersFromFaceAfeka=false;
      
       if ( movement.rivalName!=player.playername)
          MatchBetweenPlayersFromFaceAfeka=false;

      if(movement.gameCode!=player.gameCode)
          MatchBetweenPlayersFromFaceAfeka=false;
  ...
  if(MatchBetweenPlayersFromFaceAfeka)
  {
  //there is more then one player online and all players are pressed ready
  if (((TotalPlayers == numOfPlayersToStartAGame) && (TotalNotReadyPlayers == 0) && TotalTeamPlayer == 0) || (canPlay)) {
...
//Can be: background, image, text , player(bird)
